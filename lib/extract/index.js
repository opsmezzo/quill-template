...
//
// ### @private function list (fullpath, config, callback)
// #### @fullpath {string|Array} Full path(s) to the dir or file to extract from.
// #### @config {Object} **Optional** config values to use to lookup key refs.
// #### @callback {function} Continuation to respond to when complete
...
function list(fullpath, config, callback) {
  ...
  //
  // If we are passed an array of files or directories
  // then execute on all of them reducing the sets.
  //
  if (Array.isArray(fullpath)) {
    return async.reduce(fullpath, {
      required: [],
      optional: [],
      fatal: [],
      warn: []
    }, function (values, loc, next) {
      list.call(that, loc, config, function (err, result) {
        return !err
          ? next(null, addValues({ iter: result || {} }, values))
          : next(err);
      });
    }, callback);
  }
