  ...
  async.map(
    ...
    function (name, next) {
      process.nextTick(function () {
        emitter.emit('fetch', name);
      });

      target.get(name, next);
    },
    function (err, data) {
      if (err) {
        return callback(err);
...
exports.getConfig = function (options, callback) {
  ...
  remotes.filter(Boolean).forEach(function (config) {
    ...
    if (equals !== -1) {
      path = config.substr(0, equals).split(':');
      ...
      key = path.shift();
      target[key] = config.substr(equals + 1);
      return;
    }
  ...
  return fetch(options.client, sets, function (err, configs) {
    if (err) {
      return callback(err);
    }
...
exports.getEnv = function (options, callback) {
  return exports.getConfig(options, function (err, config) {
    return err
      ? callback(err)
